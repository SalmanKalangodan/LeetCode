...
var kthDistinct = function(arr, k) {
    ...
    for(i=0; i<arr.length; i++){
        if(str !== arr[i]&&!narr.includes(arr[i])){
            str = arr[i]
            narr.push(arr[i])
        }else if(narr.includes(arr[i])){
            const index= narr.indexOf(arr[i])
            narr.splice(index,1)
        } else{
            str = arr[i]
        }
    }
    const result = narr[k-1]
    if(result === undefined){
        return ""
    }
    return result
